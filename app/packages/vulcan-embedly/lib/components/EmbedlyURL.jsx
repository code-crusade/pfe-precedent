...
class EmbedlyURL extends Component {
  ...
  clearThumbnail() {
    if (
      confirm(this.context.intl.formatMessage({ id: 'posts.clear_thumbnail?' }))
    ) {
  ...
  async handleBlur() {
    try {
      ...
      if (url.length) {
        ...
        const {
          data: { getEmbedData: { title, description, thumbnailUrl } },
        } = result;
      ...
      <div className="embedly-thumbnail">
        ...
        <div className="embedly-thumbnail-actions">
          ...
          <a className="thumbnail-clear" onClick={this.clearThumbnail}>
            ...
            <FormattedMessage id="posts.clear_thumbnail" />
          </a>
        </div>
      </div>
    );
  ...
  renderNoThumbnail() {
    return (
      <div className="embedly-thumbnail">
        <div
          style={{
            width: `${Math.round(60 * this.getDimensions().ratio)}px`,
            height: `60px`,
          }}
